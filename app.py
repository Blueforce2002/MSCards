...
def get_notify_consent():
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            row = cur.fetchone()
    return jsonify({"consent": bool(row["consent"]) if row else False})
...
def send_email(to_email, subject, html_body):
    response = requests.post(
        ...
        json={
            "from": "MSCards <onboarding@resend.dev>",
            "to": [to_email],
            "subject": subject,
            "html": html_body,
        },
...
def notify_check():
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT cardmarket_name, card_number, image_url FROM set_cards WHERE id = %s;", (card_id,))
            card = cur.fetchone()
            if not card:
    ...
    for m in matches:
        ...
        html = f"""
            ...
            <p><a href="https://mscards.dk">Se det på mscards.dk</a></p>
        """
        status, body = send_email(m["email"], subject, html)
